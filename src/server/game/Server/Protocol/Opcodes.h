...
enum Opcodes
{
    ...
    //WOTLK
    CMSG_MOVE_CHARACTER_CHEAT                       = 0x701,
    SMSG_MOVE_CHARACTER_CHEAT                       = 0x702,
    SMSG_REFER_A_FRIEND_EXPIRED                     = 0x703,
    SMSG_DESTRUCTIBLE_BUILDING_DAMAGE               = 0x704,
    CMSG_DESTROY_ITEMS                              = 0x705,
    ...
    SMSG_BATTLEFIELD_PORT_DENIED                    = 0x714,
    CMSG_PERFORM_ACTION_SET                         = 0x715,
    SMSG_RESUME_CAST_BAR                            = 0x716,
    ...
    SMSG_CHANGEPLAYER_DIFFICULTY_RESULT             = 0x725,
    SMSG_UPDATE_INSTANCE_ENCOUNTER_UNIT             = 0x726,
    SMSG_QUEST_FORCE_REMOVE                         = 0x727, // uint32 questid
    SMSG_FORCE_SET_VEHICLE_REC_ID                   = 0x728,
    CMSG_SET_VEHICLE_REC_ID_ACK                     = 0x729,
    SMSG_COMBAT_EVENT_FAILED                        = 0x730,
    CMSG_SET_SAVED_INSTANCE_EXTEND                  = 0x731,
    SMSG_LFG_OFFER_CONTINUE                         = 0x732,
    CMSG_TEST_DROP_RATE                             = 0x733,
    SMSG_TEST_DROP_RATE_RESULT                      = 0x734,
    CMSG_LFG_GET_STATUS                             = 0x735,
    SMSG_SHOW_MAILBOX                               = 0x736,
    SMSG_RESET_RANGED_COMBAT_TIMER                  = 0x737,
    SMSG_CHAT_NOT_IN_PARTY                          = 0x738, // uint32, errors: ERR_NOT_IN_GROUP (2, 51) and ERR_NOT_IN_RAID (3, 39, 40)
    MSG_DEV_SHOWLABEL                               = 0x739,
    CMSG_SHOWING_HELM                               = 0x740,
    CMSG_SHOWING_CLOAK                              = 0x741,
    SMSG_LFG_ROLE_CHOSEN                            = 0x742,
    SMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY= 0x743,
    SMSG_MOVE_UNSET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY= 0x744,
    CMSG_MOVE_SET_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY_ACK= 0x745,
    MSG_MOVE_UPDATE_CAN_TRANSITION_BETWEEN_SWIM_AND_FLY= 0x745,
    CMSG_LFG_JOIN                                   = 0x747,
    CMSG_LFG_LEAVE                                  = 0x748,
    CMSG_SEARCH_LFG_JOIN                            = 0x749,
    CMSG_SEARCH_LFG_LEAVE                           = 0x750,
    SMSG_UPDATE_LFG_LIST                            = 0x751,    // uint32, uint32, if (uint8) { uint32 count, for (count) { uint64} }, uint32 count2, uint32, for (count2) { uint64, uint32 flags, if (flags & 0x2) {string}, if (flags & 0x10) {for (3) uint8}, if (flags & 0x80) {uint64, uint32}}, uint32 count3, uint32, for (count3) {uint64, uint32 flags, if (flags & 0x1) {uint8, uint8, uint8, for (3) uint8, uint32, uint32, uint32, uint32, uint32, uint32, float, float, uint32, uint32, uint32, uint32, uint32, float, uint32, uint32, uint32, uint32, uint32, uint32}, if (flags&0x2) string, if (flags&0x4) uint8, if (flags&0x8) uint64, if (flags&0x10) uint8, if (flags&0x20) uint32, if (flags&0x40) uint8, if (flags& 0x80) {uint64, uint32}}
    SMSG_LFG_PROPOSAL_UPDATE                        = 0x752,    // uint32, uint8, uint32, uint32, uint8, for (uint8) {uint32, uint8, uint8, uint8, uint8}
    CMSG_LFG_PROPOSAL_RESULT                        = 0x753,
    SMSG_LFG_ROLE_CHECK_UPDATE                      = 0x754,    // uint32, uint8, for (uint8) uint32, uint8, for (uint8) { uint64, uint8, uint32, uint8, }
    SMSG_LFG_JOIN_RESULT                            = 0x755,    // uint32 unk, uint32, if (unk == 6) { uint8 count, for (count) uint64 }
    SMSG_LFG_QUEUE_STATUS                           = 0x756,    // uint32 dungeon, uint32 lfgtype, uint32, uint32, uint32, uint32, uint8, uint8, uint8, uint8
    SMSG_LFG_UPDATE_PLAYER                          = 0x757,    // uint8, if (uint8) { uint8, uint8, uint8, uint8, if (uint8) for (uint8) uint32, string}
    SMSG_LFG_UPDATE_PARTY                           = 0x758,    // uint8, if (uint8) { uint8, uint8, uint8, for (3) uint8, uint8, if (uint8) for (uint8) uint32, string}
    SMSG_LFG_UPDATE_SEARCH                          = 0x759,    // uint8
    CMSG_LFG_SET_ROLES                              = 0x760,
    CMSG_LFG_SET_NEEDS                              = 0x761,
    CMSG_LFG_SET_BOOT_VOTE                          = 0x762,
    SMSG_LFG_BOOT_PROPOSAL_UPDATE                   = 0x763,    // uint8, uint8, uint8, uint64, uint32, uint32, uint32, uint32
    CMSG_LFD_PLAYER_LOCK_INFO_REQUEST               = 0x764,
    SMSG_LFG_PLAYER_INFO                            = 0x765,    // uint8, for (uint8) { uint32, uint8, uint32, uint32, uint32, uint32, uint8, for (uint8) {uint32, uint32, uint32}}, uint32, for (uint32) {uint32, uint32}
    CMSG_LFG_TELEPORT                               = 0x766,
    CMSG_LFD_PARTY_LOCK_INFO_REQUEST                = 0x767,
    SMSG_LFG_PARTY_INFO                             = 0x768,    // uint8, for (uint8) uint64
    SMSG_VOICE_SESSION_ENABLE                       = 0x769,
    CMSG_CHANGE_PERSONAL_ARENA_RATING               = 0x770,
    SMSG_MULTIPLE_MOVES                             = 0x771,
    CMSG_ALTER_APPEARANCE                           = 0x772,
    SMSG_ENABLE_BARBER_SHOP                         = 0x773,
    SMSG_BARBER_SHOP_RESULT                         = 0x774,
    CMSG_CALENDAR_GET_CALENDAR                      = 0x775,
    CMSG_CALENDAR_GET_EVENT                         = 0x776,
    CMSG_CALENDAR_GUILD_FILTER                      = 0x777,
    CMSG_CALENDAR_ARENA_TEAM                        = 0x778,
    CMSG_CALENDAR_ADD_EVENT                         = 0x779,
    CMSG_CALENDAR_UPDATE_EVENT                      = 0x780,
    CMSG_CALENDAR_REMOVE_EVENT                      = 0x781,
    CMSG_CALENDAR_COPY_EVENT                        = 0x782,
    CMSG_CALENDAR_EVENT_INVITE                      = 0x783,
    CMSG_CALENDAR_EVENT_RSVP                        = 0x784,
    CMSG_CALENDAR_EVENT_REMOVE_INVITE               = 0x785,
    CMSG_CALENDAR_EVENT_STATUS                      = 0x786,
    CMSG_CALENDAR_EVENT_MODERATOR_STATUS            = 0x787,
    SMSG_CALENDAR_SEND_CALENDAR                     = 0x788,
    SMSG_CALENDAR_SEND_EVENT                        = 0x789,
    SMSG_CALENDAR_FILTER_GUILD                      = 0x790,
    SMSG_CALENDAR_ARENA_TEAM                        = 0x791,
    SMSG_CALENDAR_EVENT_INVITE                      = 0x792,
    SMSG_CALENDAR_EVENT_INVITE_REMOVED              = 0x793,
    SMSG_CALENDAR_EVENT_STATUS                      = 0x794,
    SMSG_CALENDAR_COMMAND_RESULT                    = 0x795,
    SMSG_CALENDAR_RAID_LOCKOUT_ADDED                = 0x796,
    SMSG_CALENDAR_RAID_LOCKOUT_REMOVED              = 0x797,
    SMSG_CALENDAR_EVENT_INVITE_ALERT                = 0x798,
    SMSG_CALENDAR_EVENT_INVITE_REMOVED_ALERT        = 0x799,
    SMSG_CALENDAR_EVENT_INVITE_STATUS_ALERT         = 0x800,
    SMSG_CALENDAR_EVENT_REMOVED_ALERT               = 0x801,
    SMSG_CALENDAR_EVENT_UPDATED_ALERT               = 0x802,
    SMSG_CALENDAR_EVENT_MODERATOR_STATUS_ALERT      = 0x803,
    CMSG_CALENDAR_COMPLAIN                          = 0x804,
    CMSG_CALENDAR_GET_NUM_PENDING                   = 0x805,
    SMSG_CALENDAR_SEND_NUM_PENDING                  = 0x806,
    CMSG_SAVE_DANCE                                 = 0x807,
    SMSG_NOTIFY_DANCE                               = 0x808,
    CMSG_PLAY_DANCE                                 = 0x809,
    SMSG_PLAY_DANCE                                 = 0x810,
    CMSG_LOAD_DANCES                                = 0x811,
    CMSG_STOP_DANCE                                 = 0x812,
    SMSG_STOP_DANCE                                 = 0x813,
    CMSG_SYNC_DANCE                                 = 0x814,
    CMSG_DANCE_QUERY                                = 0x815,
    SMSG_DANCE_QUERY_RESPONSE                       = 0x816,
    SMSG_INVALIDATE_DANCE                           = 0x817,
    CMSG_DELETE_DANCE                               = 0x818,
    //SMSG_LEARNED_DANCE_MOVES                        = 0x819,
    CMSG_LEARN_DANCE_MOVE                           = 0x820,
    CMSG_UNLEARN_DANCE_MOVE                         = 0x821,
    CMSG_XXXXXXXXXXXXXXX                            = 0x822,
    CMSG_SET_RUNE_COUNT                             = 0x823,
    CMSG_SET_RUNE_COOLDOWN                          = 0x824,
    MSG_MOVE_SET_PITCH_RATE_CHEAT                   = 0x825,
    MSG_MOVE_SET_PITCH_RATE                         = 0x826,
    SMSG_FORCE_PITCH_RATE_CHANGE                    = 0x827,
    CMSG_FORCE_PITCH_RATE_CHANGE_ACK                = 0x828,
    SMSG_SPLINE_SET_PITCH_RATE                      = 0x829,
    CMSG_CALENDAR_EVENT_INVITE_NOTES                = 0x830,
    SMSG_CALENDAR_EVENT_INVITE_NOTES                = 0x831,
    SMSG_CALENDAR_EVENT_INVITE_NOTES_ALERT          = 0x832,
    CMSG_UPDATE_MISSILE_TRAJECTORY                  = 0x833,
    SMSG_UPDATE_ACCOUNT_DATA_COMPLETE               = 0x834,
    SMSG_TRIGGER_MOVIE                              = 0x835,
    CMSG_COMPLETE_MOVIE                             = 0x836,
    CMSG_SET_GLYPH_SLOT                             = 0x837,
    CMSG_SET_GLYPH                                  = 0x838,
    //SMSG_ACHIEVEMENT_EARNED                         = 0x839,
    SMSG_DYNAMIC_DROP_ROLL_RESULT                   = 0x840,
    //SMSG_CRITERIA_UPDATE                            = 0x841,
    CMSG_QUERY_INSPECT_ACHIEVEMENTS                 = 0x842,
    //SMSG_RESPOND_INSPECT_ACHIEVEMENTS               = 0x843,
    CMSG_DISMISS_CONTROLLED_VEHICLE                 = 0x844,
    CMSG_COMPLETE_ACHIEVEMENT_CHEAT                 = 0x845,
    SMSG_QUESTUPDATE_ADD_PVP_KILL                   = 0x846,
    CMSG_SET_CRITERIA_CHEAT                         = 0x847,
    SMSG_CALENDAR_RAID_LOCKOUT_UPDATED              = 0x848,
    CMSG_UNITANIMTIER_CHEAT                         = 0x849,
    CMSG_CHAR_CUSTOMIZE                             = 0x850,
    SMSG_CHAR_CUSTOMIZE                             = 0x851,
    SMSG_PET_RENAMEABLE                             = 0x852,
    CMSG_REQUEST_VEHICLE_EXIT                       = 0x853,
    CMSG_REQUEST_VEHICLE_PREV_SEAT                  = 0x854,
    CMSG_REQUEST_VEHICLE_NEXT_SEAT                  = 0x855,
    CMSG_REQUEST_VEHICLE_SWITCH_SEAT                = 0x856,
    CMSG_PET_LEARN_TALENT                           = 0x857,
    CMSG_PET_UNLEARN_TALENTS                        = 0x858,
    SMSG_SET_PHASE_SHIFT                            = 0x859,
    //SMSG_ALL_ACHIEVEMENT_DATA                       = 0x860,
    CMSG_FORCE_SAY_CHEAT                            = 0x861,
    //SMSG_HEALTH_UPDATE                              = 0x862,
    //SMSG_POWER_UPDATE                               = 0x863,
    CMSG_GAMEOBJ_REPORT_USE                         = 0x864,
    SMSG_HIGHEST_THREAT_UPDATE                      = 0x865,
    SMSG_THREAT_UPDATE                              = 0x866,
    SMSG_THREAT_REMOVE                              = 0x867,
    SMSG_THREAT_CLEAR                               = 0x868,
    SMSG_CONVERT_RUNE                               = 0x869,
    SMSG_RESYNC_RUNES                               = 0x870,
    SMSG_ADD_RUNE_POWER                             = 0x871,
    CMSG_START_QUEST                                = 0x872,
    CMSG_REMOVE_GLYPH                               = 0x873,
    CMSG_DUMP_OBJECTS                               = 0x874,
    SMSG_DUMP_OBJECTS_DATA                          = 0x875,
    CMSG_DISMISS_CRITTER                            = 0x876,
    SMSG_NOTIFY_DEST_LOC_SPELL_CAST                 = 0x877,
    CMSG_AUCTION_LIST_PENDING_SALES                 = 0x878,
    SMSG_AUCTION_LIST_PENDING_SALES                 = 0x879,
    SMSG_MODIFY_COOLDOWN                            = 0x880,
    SMSG_PET_UPDATE_COMBO_POINTS                    = 0x881,
    CMSG_ENABLETAXI                                 = 0x882,
    SMSG_PRE_RESURRECT                              = 0x883,
    //SMSG_AURA_UPDATE_ALL                            = 0x884,
    //SMSG_AURA_UPDATE                                = 0x885,
    CMSG_FLOOD_GRACE_CHEAT                          = 0x886,
    //SMSG_SERVER_FIRST_ACHIEVEMENT                   = 0x887,
    SMSG_PET_LEARNED_SPELL                          = 0x888,
    SMSG_PET_REMOVED_SPELL                          = 0x889,
    CMSG_CHANGE_SEATS_ON_CONTROLLED_VEHICLE         = 0x890,
    CMSG_HEARTH_AND_RESURRECT                       = 0x891,
    SMSG_ON_CANCEL_EXPECTED_RIDE_VEHICLE_AURA       = 0x892,
    //SMSG_CRITERIA_DELETED                           = 0x893,
    //SMSG_ACHIEVEMENT_DELETED                        = 0x894,
    CMSG_SERVER_INFO_QUERY                          = 0x895,
    SMSG_SERVER_INFO_RESPONSE                       = 0x896,
    CMSG_CHECK_LOGIN_CRITERIA                       = 0x897,
    SMSG_SERVER_BUCK_DATA_START                     = 0x898,
    CMSG_SET_BREATH                                 = 0x899,
    CMSG_QUERY_VEHICLE_STATUS                       = 0x900,
    SMSG_BATTLEGROUND_INFO_THROTTLED                = 0x901, // empty, "You can't do that yet"
    SMSG_PLAYER_VEHICLE_DATA                        = 0x902, // guid+uint32 (vehicle)
    CMSG_PLAYER_VEHICLE_ENTER                       = 0x903, // uint64
    CMSG_CONTROLLER_EJECT_PASSENGER                 = 0x904, // uint64
    SMSG_PET_GUIDS                                  = 0x905,
    //SMSG_CLIENTCACHE_VERSION                        = 0x906,
    CMSG_CHANGE_GDF_ARENA_RATING                    = 0x907,
    CMSG_SET_ARENA_TEAM_RATING_BY_INDEX             = 0x908,
    CMSG_SET_ARENA_TEAM_WEEKLY_GAMES                = 0x909,
    CMSG_SET_ARENA_TEAM_SEASON_GAMES                = 0x910,
    CMSG_SET_ARENA_MEMBER_WEEKLY_GAMES              = 0x911,
    CMSG_SET_ARENA_MEMBER_SEASON_GAMES              = 0x912,
    SMSG_ITEM_REFUND_INFO_RESPONSE                  = 0x913,
    ...
    CMSG_GM_GRANT_ACHIEVEMENT                       = 0x931,
    CMSG_GM_REMOVE_ACHIEVEMENT                      = 0x932,
    CMSG_GM_SET_CRITERIA_FOR_PLAYER                 = 0x933,
    SMSG_ARENA_UNIT_DESTROYED                       = 0x934,
    SMSG_ARENA_TEAM_CHANGE_FAILED_QUEUED            = 0x935, // uint32 "Can't modify arena team while queued or in a match."
    CMSG_PROFILEDATA_REQUEST                        = 0x936,
    SMSG_PROFILEDATA_RESPONSE                       = 0x937,
    CMSG_START_BATTLEFIELD_CHEAT                    = 0x938,
    CMSG_END_BATTLEFIELD_CHEAT                      = 0x939,
    SMSG_MULTIPLE_PACKETS                           = 0x940,
    SMSG_MOVE_GRAVITY_DISABLE                       = 0x941,
    CMSG_MOVE_GRAVITY_DISABLE_ACK                   = 0x942,
    SMSG_MOVE_GRAVITY_ENABLE                        = 0x943,
    CMSG_MOVE_GRAVITY_ENABLE_ACK                    = 0x944,
    ...
    SMSG_CHAR_FACTION_CHANGE                        = 0x953,
    CMSG_PVP_QUEUE_STATS_REQUEST                    = 0x954,
    SMSG_PVP_QUEUE_STATS                            = 0x955,
    CMSG_SET_PAID_SERVICE_CHEAT                     = 0x956,
    SMSG_BATTLEFIELD_MGR_ENTRY_INVITE               = 0x957, // uint32
    CMSG_BATTLEFIELD_MGR_ENTRY_INVITE_RESPONSE      = 0x958,
    SMSG_BATTLEFIELD_MGR_ENTERED                    = 0x959, // uint32, uint8, uint8
    SMSG_BATTLEFIELD_MGR_QUEUE_INVITE               = 0x960, // uint32
    CMSG_BATTLEFIELD_MGR_QUEUE_INVITE_RESPONSE      = 0x961,
    CMSG_BATTLEFIELD_MGR_QUEUE_REQUEST              = 0x962,
    SMSG_BATTLEFIELD_MGR_QUEUE_REQUEST_RESPONSE     = 0x963, // uint32, uint8
    SMSG_BATTLEFIELD_MGR_EJECT_PENDING              = 0x964, // uint32
    SMSG_BATTLEFIELD_MGR_EJECTED                    = 0x965, // uint32, uint32, uint8
    CMSG_BATTLEFIELD_MGR_EXIT_REQUEST               = 0x966,
    SMSG_BATTLEFIELD_MGR_STATE_CHANGE               = 0x967, // uint32, uint32
    CMSG_BATTLEFIELD_MANAGER_ADVANCE_STATE          = 0x968,
    CMSG_BATTLEFIELD_MANAGER_SET_NEXT_TRANSITION_TIME= 0x969,
    MSG_SET_RAID_DIFFICULTY                         = 0x970,
    CMSG_TOGGLE_XP_GAIN                             = 0x971,
    SMSG_TOGGLE_XP_GAIN                             = 0x972, // enable/disable XP gain console message
    SMSG_GMRESPONSE_DB_ERROR                        = 0x973, // empty
    SMSG_GMRESPONSE_RECEIVED                        = 0x974, // uint32, uint32, string[2000], string[4000][4]
    CMSG_GMRESPONSE_RESOLVE                         = 0x975,
    SMSG_GMRESPONSE_STATUS_UPDATE                   = 0x976, // uint8 (1 - EVENT_GMSURVEY_DISPLAY, 0 - EVENT_UPDATE_TICKET)
    SMSG_GMRESPONSE_CREATE_TICKET                   = 0x977,
    CMSG_GMRESPONSE_CREATE_TICKET                   = 0x978,
    CMSG_SERVERINFO                                 = 0x979,
    SMSG_SERVERINFO                                 = 0x980,
    CMSG_WORLD_STATE_UI_TIMER_UPDATE                = 0x981,
    SMSG_WORLD_STATE_UI_TIMER_UPDATE                = 0x982,
    CMSG_CHAR_RACE_CHANGE                           = 0x983,
    MSG_VIEW_PHASE_SHIFT                            = 0x984,
    SMSG_TALENTS_INVOLUNTARILY_RESET                = 0x985, // uint8
    CMSG_DEBUG_SERVER_GEO                           = 0x986,
    SMSG_DEBUG_SERVER_GEO                           = 0x987,
    SMSG_LOOT_SLOT_CHANGED                          = 0x988,
    UMSG_UPDATE_GROUP_INFO                          = 0x989,
    CMSG_READY_FOR_ACCOUNT_DATA_TIMES               = 0x990,
    CMSG_QUERY_QUESTS_COMPLETED                     = 0x991,
    SMSG_QUERY_QUESTS_COMPLETED_RESPONSE            = 0x992,
    CMSG_GM_REPORT_LAG                              = 0x993,
    CMSG_AFK_MONITOR_INFO_REQUEST                   = 0x994,
    SMSG_AFK_MONITOR_INFO_RESPONSE                  = 0x995,
    CMSG_AFK_MONITOR_INFO_CLEAR                     = 0x996,
    SMSG_CORPSE_NOT_IN_INSTANCE                     = 0x997,
    ...
    CMSG_REDIRECTION_FAILED                         = 0x9A6, // something with networking
    SMSG_SUSPEND_COMMS                              = 0x9A7,
    CMSG_SUSPEND_COMMS_ACK                          = 0x9A8,
    SMSG_FORCE_SEND_QUEUED_PACKETS                  = 0x9A9,
    CMSG_REDIRECTION_AUTH_PROOF                     = 0x9B1,
    CMSG_DROP_NEW_CONNECTION                        = 0x9B2,
    SMSG_SEND_ALL_COMBAT_LOG                        = 0x9B3,
    SMSG_OPEN_LFG_DUNGEON_FINDER                    = 0x9B4,
    SMSG_MOVE_SET_COLLISION_HGT                     = 0x9B5,
    CMSG_MOVE_SET_COLLISION_HGT_ACK                 = 0x9B6,
    MSG_MOVE_SET_COLLISION_HGT                      = 0x9B7,
    CMSG_CLEAR_RANDOM_BG_WIN_TIME                   = 0x9B8,
    CMSG_CLEAR_HOLIDAY_BG_WIN_TIME                  = 0x9B9,
    CMSG_COMMENTATOR_SKIRMISH_QUEUE_COMMAND         = 0x9C1,
    SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT1         = 0x9C2,
    SMSG_COMMENTATOR_SKIRMISH_QUEUE_RESULT2         = 0x9C3,
};
