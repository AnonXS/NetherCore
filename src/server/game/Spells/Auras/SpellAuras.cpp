...
_flags(AFLAG_NONE), _effectsToApply(effMask), _needClientUpdate(false)
{
    ...
    if (GetBase()->CanBeSentToClient())
    {
        ...
        else
        {
            ...
            uint8 freeSlot = 0;
            uint8 maxSlot = MAX_POSITIVE_AURAS;

            if (!IsPositive())
            {
                freeSlot = MAX_POSITIVE_AURAS;
                maxSlot = MAX_AURAS;
            }

            for (freeSlot; freeSlot < maxSlot; ++freeSlot)
            {
                if (visibleAuras->find(freeSlot) == visibleAuras->end())
                {
                    slot = freeSlot;
                    break;
                }
...
void AuraApplication::SetAuraLevel(uint32 slot, uint32 level)
{
    uint32 index        = slot / 4;
    uint32 byte         = (slot % 4) * 8;
    uint32 val          = GetTarget()->GetUInt32Value(UNIT_FIELD_AURALEVELS + index);
...
void AuraApplication::UpdateAuraApplication()
{
    ...
    val |= ((uint8(stackAmount <= 255 ? stackAmount - 1 : 255 - 1)) << byte);

    GetTarget()->SetUInt32Value(UNIT_FIELD_AURAAPPLICATIONS + index, val);
}
            ...
            case SPELLFAMILY_MAGE:
                ...
                switch (GetId())
                {
                    ...
                    case 12043: // Presence of Mind
                        // Arcane Potency
                        if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 2120, uint8(0)))
                        {
                            uint32 spellId = 0;
    ...
    else
    {
        switch (GetSpellInfo()->SpellFamilyName)
        {
                ...
                if (GetSpellInfo()->SpellFamilyFlags[1] & 0x2)
                {
                    if (removeMode != AURA_REMOVE_BY_DEFAULT)
                    {
                        ...
                        if (caster->GetDummyAuraEffect(SPELLFAMILY_WARRIOR, 1935, 1))
                        {
                            ...
                            {
                                if ((*itr)!= target)
                                    (*itr)->RemoveAurasWithFamily(SPELLFAMILY_WARRIOR, 0, 0x2, GetCasterGUID());
                            }
                        }
                    }
                }
            ...
            case SPELLFAMILY_WARLOCK:
                ...
                {
                    if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_WARLOCK, 98, uint8(0)))
                    {
                        uint32 spellId = 0;
                        switch (aurEff->GetId())
                        {
