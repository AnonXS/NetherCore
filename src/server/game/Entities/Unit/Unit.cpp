...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo /*= NULL*/)
{
    ...
    // Ignore Absorption Auras (not implemented in 2.4.3)
...
void Unit::RemoveAurasWithFamily(SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint64 casterGUID)
{
    for (AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        Aura const* aura = iter->second->GetBase();
...
AuraEffect* Unit::GetAuraEffect(AuraType type, SpellFamilyNames family, uint32 familyFlag1, uint32 familyFlag2, uint64 casterGUID) const
{
    AuraEffectList const& auras = GetAuraEffectsByType(type);
...
void Unit::SendAttackStateUpdate(CalcDamageInfo* damageInfo)
{
    ...
    WorldPacket data(SMSG_ATTACKERSTATEUPDATE, (4+8+8+4+1+4+4+4+4+4+4+4+4+4+4));
    data << uint32(damageInfo->HitInfo);
    data.append(damageInfo->attacker->GetPackGUID());
    data.append(damageInfo->target->GetPackGUID());
    data << uint32(damageInfo->damage);                     // Full damage

    data << uint8(1);                                       // Sub damage
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummySpell->SpellFamilyFlags.IsEqual(0, 0) && dummySpell->SpellIconID == 1932)
            {
            ...
            switch (dummySpell->SpellIconID)
            {
                case 2236: // Thrill of the Hunt
                {
                    ...
                    if (procSpell->SpellFamilyFlags[2] & 0x200)
                    {
                        if (!victim)
                            return false;
                        if (AuraEffect const* pEff = victim->GetAuraEffect(SPELL_AURA_PERIODIC_DUMMY, SPELLFAMILY_HUNTER, 0x0, 0x80000000, GetGUID()))
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch (dummySpell->Id)
            {
                ...
                case 70817:
                {
                    ...
                    if (AuraEffect const* aurEff = target->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0x0, GetGUID()))
                    {
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 72176:
            basepoints0 = 3;
            break;
        /* 2.4.3 server crash proc on fall dmg
...
void Unit::SendHealSpellLog(Unit* victim, uint32 SpellID, uint32 Damage, uint32 OverHeal, bool critical)
{
    // we guess size
...
int32 Unit::HealBySpell(Unit* victim, SpellInfo const* spellInfo, uint32 addHealth, bool critical)
{
    ...
    SendHealSpellLog(victim, spellInfo->Id, addHealth, uint32(addHealth - gain), critical);
    return gain;
}
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch ((*i)->GetMiscValue())
        {
            ...
            case 7377:
            {
                if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, 0x8000, 0, GetGUID()))
                    AddPct(DoneTotalMod, (*i)->GetAmount());
                break;
            }
            // Marked for Death
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PRIEST:
            ...
            if (spellProto->SpellFamilyFlags[0] & 0x800000)
                ...
                if (AuraEffect* aurEff = GetAuraEffect(SPELL_AURA_OVERRIDE_CLASS_SCRIPTS, SPELLFAMILY_PRIEST, 2848, uint8(1)))
            ...
            if (victim)
            {
                ...
                switch (spellProto->SpellFamilyName)
                {
                    ...
                    case SPELLFAMILY_SHAMAN:
                        ...
                        {
                            if (victim->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_SHAMAN, 0x10000000, 0, GetGUID()))
    ...
    switch (mtype)
    {
        ...
        case MOVE_FLIGHT:
        {
            ...
            else if (IsMounted())
            {
                ...
                main_speed_mod  = GetTotalAuraModifier(SPELL_AURA_MOD_FLIGHT_SPEED_STACKING) + GetTotalAuraModifier(SPELL_AURA_MOD_FLIGHT_SPEED);
...
{
    return isType(TYPEMASK_PLAYER) && ToPlayer()->IsFFAPvP();
}
