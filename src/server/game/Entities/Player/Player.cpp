...
void Player::SetDrunkValue(uint8 newDrunkValue, uint32 itemId /*= 0*/)
{
    ...
    if (newDrunkValue)
    {
        ...
        m_invisibilityDetect.SetValue(INVISIBILITY_DRUNK, newDrunkValue);
    }
    else
...
InventoryResult Player::CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec &dest, uint32 entry, uint32 count, Item* pItem, bool swap, uint32* no_space_count) const
{
    ...
    if (bag != NULL_BAG)
    {
        ...
        if (pProto->Stackable != 1)
        {
            ...
            {
                res = CanStoreItem_InInventorySlots(KEYRING_SLOT_START, KEYRING_SLOT_END, dest, pProto, count, true, pItem, bag, slot);
                if (res != EQUIP_ERR_OK)
                {
                    if (no_space_count)
                        *no_space_count = count + no_similar_count;
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    // cheating attempt
    if (count < 1) count = 1;
...
uint32 Player::GetResurrectionSpellId()
{
    ...
    if (prio < 1 && HasSpell(20608) && !HasSpellCooldown(21169) && HasItemCount(17030))
        spell_id = 21169;
